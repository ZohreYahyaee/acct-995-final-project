...
-   Despite the scale, only 9,159 SKUs received at least one order during the observation window, highlighting the skewed nature of demand.

-   A total of 1,123 SKUs exited the platform during the sample period, out of 31,876 observed SKUs.
...
```{r Filter to 3P SKUs and define target variable, include=FALSE, message = FALSE}
...
```{r}
#| label: fig-model-performance-summary
#| fig-cap: "Performance comparison across all workflows based on accuracy, precision, recall, F1-score, AUC, and Brier score."
#| echo: false
#| include: true
#| message: false
#| warning: false
...
:::
Performance comparison across all workflows based on accuracy, precision, recall, F1-score, AUC, and Brier score.
...
@fig-conf-matrix displays the confusion matrix generated from the final XGBoost model’s predictions on the test set. The counts are as follows:
...
#| fig-cap: "Confusion matrix of XGBoost predictions on the test set, showing counts of true positives, true negatives, false positives, and false negatives."
#| echo: false
#| include: true
#| message: false
#| warning: false
...
The metrics reported in @tbl-eval-metrics-table provide a broader view of the model’s predictive performance:
...
```{r}
#| label: tbl-eval-metrics-table
#| tbl-cap: "Evaluation metrics for the final XGBoost model, including accuracy, precision, recall, F1-score, and ROC AUC."
#| echo: false
#| include: true
#| message: false
#| warning: false
...
@fig-custom-permutation-vi shows the variable importance results under a permutation scheme using a binary 0/1 loss function. Variables such as max_non_interacting_streak, total_sku_type_2_in_product_space, and click_quantile_overall are among the most important predictors, confirming earlier findings that disengagement and competitive intensity are key drivers of SKU exit.
...
```{r}
#| label: fig-custom-permutation-vi
#| fig-cap: "Permutation-based variable importance using a binary 0/1 loss function. Variables with higher dropout loss are more important to the XGBoost model's prediction of third-party SKU exit."
#| echo: false
#| include: true
#| message: false
#| warning: false
...
@fig-pdp-top: Partial Dependence Profiles (PDPs) for the six most influential predictors in the XGBoost model.
...
```{r}
#| label: fig-pdp-top
#| fig-cap: "Partial Dependence Profiles (PDPs) for the six most influential predictors in the XGBoost model. These plots show how each variable affects the predicted probability of SKU exit, averaged over all other features."
#| echo: false
#| message: false
#| warning: false
#| results: 'hide'
